...
async def get_data():
    ...
    if not os.path.exists(cleaned_path) or not os.path.exists(forecast_path):
        ...
        raise HTTPException(
            status_code=404, detail="Data files not found. Run the pipeline first."
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error reading data"
        )
...
async def predict(request: PredictRequest):
    ...
    if len(request.last_12_months) != 12:
        raise HTTPException(
            status_code=400, detail="Input must contain exactly 12 months of data"
        )
