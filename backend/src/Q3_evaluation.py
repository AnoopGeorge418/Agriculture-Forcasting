...
def evaluate_models(df, model_dir=None, plot_dir=None):
    ...
    metrics.append({'Model': 'SARIMA', 'MAE': mean_absolute_error(actuals, sarima_pred), 'RMSE': np.sqrt(mean_squared_error(actuals, sarima_pred)), 'MAPE': mean_absolute_percentage_error(actuals, sarima_pred)})
    ...
    metrics.append({'Model': 'XGBoost', 'MAE': mean_absolute_error(actuals, xgb_pred), 'RMSE': np.sqrt(mean_squared_error(actuals, xgb_pred)), 'MAPE': mean_absolute_percentage_error(actuals, xgb_pred)})
    ...
    metrics.append({'Model': 'LSTM', 'MAE': mean_absolute_error(actuals[:1], lstm_pred), 'RMSE': np.sqrt(mean_squared_error(actuals[:1], lstm_pred)), 'MAPE': mean_absolute_percentage_error(actuals[:1], lstm_pred)})
...
def generate_forecast(df, model_dir=None, forecast_path=None, plot_dir=None):
    model_dir = model_dir or os.getenv('MODELS_DIR', 'models/')
    forecast_path = forecast_path or os.getenv('DATA_FORECASTS', 'data/forecasts/12_month_forecast.csv')
    plot_dir = plot_dir or os.getenv('OUTPUTS_DIR', 'outputs/plots/')
    
    os.makedirs(os.path.dirname(forecast_path), exist_ok=True)
    sarima_res = SARIMAXResults.load(os.path.join(model_dir, 'sarima_model.pkl'))
    forecast_df = sarima_res.get_forecast(steps=12).summary_frame()[['mean']].rename(columns={'mean': 'forecasted_price'})
...
if __name__ == "__main__":
    df = pd.read_csv(os.getenv('DATA_CLEANED', 'data/cleaned/cleaned_prices.csv'), index_col='date', parse_dates=True)
    evaluate_models(df)
