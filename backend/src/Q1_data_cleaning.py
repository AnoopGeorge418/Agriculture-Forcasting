...
def clean_data(input_path=None, output_path=None, plot_dir=None):
    ...
    output_path = output_path or os.getenv('DATA_CLEANED', 'data/cleaned/cleaned_prices.csv')
    ...
    df['avg_monthly_price'] = df['avg_monthly_price'].interpolate(method='time').ffill().bfill()
    
    # 3. Handle Outliers (IQR Method)
    Q1 = df['avg_monthly_price'].quantile(0.25)
    Q3 = df['avg_monthly_price'].quantile(0.75)
    IQR = Q3 - Q1
    df['avg_monthly_price'] = df['avg_monthly_price'].clip(lower=Q1 - 1.5 * IQR, upper=Q3 + 1.5 * IQR)
